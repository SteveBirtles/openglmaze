...
func makeMaze() {
	...
	for i := -MAP_CENTRE; i <= MAP_CENTRE; i++ {
		for j := -MAP_CENTRE; j <= MAP_CENTRE; j++ {
			...
			for f := 0; f < 4; f++ {
				grid[i+MAP_CENTRE][j+MAP_CENTRE].flats[f] = rand.Intn(40) + 1 //DEFAULT_FLAT
				if f == 3 {
					break
				}
				grid[i+MAP_CENTRE][j+MAP_CENTRE].walls[f] = rand.Intn(40) + 1 //DEFAULT_WALL
			}
		}
	}
	...
	for i := 0; i < MAP_CENTRE; i++ {
		for j := 0; j < MAP_CENTRE; j++ {
			if kruskalMaze[i][j].right {
				...
				for f := 0; f < 4; f++ {
					grid[i*2+2][j*2+1].flats[f] = rand.Intn(40) + 1 //DEFAULT_FLAT
					if f == 3 {
						break
					}
					grid[i*2+2][j*2+1].walls[f] = rand.Intn(40) + 1 //DEFAULT_WALL
				}
			}
			if kruskalMaze[i][j].down {
				grid[i*2+1][j*2+2].cellType = GENERATOR_PATH
				for f := 0; f < 4; f++ {
					grid[i*2+1][j*2+2].flats[f] = rand.Intn(40) + 1 //DEFAULT_FLAT
					if f == 3 {
						break
					}
					grid[i*2+1][j*2+2].walls[f] = rand.Intn(40) + 1 //DEFAULT_WALL
				}
			}
		}
	}
	...
	for r := 0; r < 10; r++ {
		...
		for i := x; i <= x+rw; i++ {
			for j := y; j <= y+rh; j++ {
				...
				for f := 0; f < 4; f++ {
					grid[i][j].flats[f] = rand.Intn(40) + 1 //DEFAULT_FLAT
				}
				for f := 0; f < 3; f++ {
					grid[i][j].walls[f] = rand.Intn(40) + 1 //DEFAULT_WALL
				}
			}
		}
	}
